...
function fluidRow(tabName, grid) {
  ...
  grid.forEach((row, ri) => {$('#shiny-tab-' + tabName).append('<div id="' + tabName + '-row' + ri+1  + '"class="row"></div>');
    // Loop through items in each row to construct box divs				     
    row.forEach((box, bi) => {$('#' + tabName + '-row' + ri+1).append('<div id="' + tabName + '-row' + ri+1 + '-box' + bi+1 + '" class="col-sm-' + box +'"><div class="box box-solid box-default"><div class="box-header"><h3 class="box-title"></h3></div><div class="box-body"></div></div></div>');} );
  })
}
