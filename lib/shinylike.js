...
// Add a dropdown list input box
// Will be added to sidebar by default, unless other home is specified eg. "#tab1-row2-box1" 
function selectInput(inputId, label, choices, home = null) {
console.log(home)
  if (home = null) { home = ".sidebar-menu"}	
  $(home).append('<div class="form-group shiny-input-container"><label class="control-label" for="select">' + label + '</label><div><select id="' + inputId + '" class="form-control"></select><script type="application/json" data-for="' + inputId + '" data-nonempty="">{}<\/script></div></div>');
  ...
  $('.sidebar-menu').append('\
      ...
      <label class="control-label" for=' + inputId + '>' + label + '</label> \
        <div class="input-daterange input-group"> \
          <input class="input-sm form-control" type="text" value=' + start + '/> \
          <span class="input-group-addon"> to </span> \
          <input class="input-sm form-control" type="text" value=' + end + '/> \
        </div> \
      </div>');
	
  $(".input-daterange").datepicker({format: "dd-mm-yyyy", startDate: "2019-01-01", endDate: "2025-01-01", autoclose: true}); 	
}
